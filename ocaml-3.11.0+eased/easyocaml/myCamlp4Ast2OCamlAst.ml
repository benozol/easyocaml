...
module Make (Ast : Sig.Camlp4Ast) = struct
  ...
  value mkloc loc : Location.t = Obj.magic (Loc.to_ocaml_location loc);
  value mkghloc loc : Location.t = Obj.magic (Loc.to_ocaml_location (Loc.ghostify loc));
  ...
  value rec expr =
    ...
    | ExApp loc e1 e2 as f ->
        ...
        | _ ->
            let module L = Ast.Loc in
            let loc' = (* this is not quite nice. but otherwise [loc = Ast.loc_of_expr e2]! *)
              let l1 = Ast.loc_of_expr e1 in
              let l2 = Ast.loc_of_expr e2 in
              if L.start_off l1 > L.start_off l2 then
                L.merge
                  (L.of_lexing_position (L.stop_pos (Ast.loc_of_expr e2)))
                  (L.of_lexing_position (L.start_pos (Ast.loc_of_expr e1)))
              else
                L.merge
                  (L.of_lexing_position (L.start_pos (Ast.loc_of_expr e1)))
                  (L.of_lexing_position (L.stop_pos (Ast.loc_of_expr e2)))
            in
            mkexp loc' (Pexp_apply (expr e1, [label_expr e2]))]
    | ExAre loc e1 e2 ->
        mkexp loc
